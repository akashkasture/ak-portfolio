/* Boot-reveal choreography for the solar-system wallpaper: stars appear,
   nebula fades in, the sun materializes, planets pop in (in real orbital
   order out from the sun), then orbital motion ramps up. Everything here
   is driven by a single 0-1 progress value (`t`) advanced by
   useIntroSequence — no per-layer timers to keep in sync by hand. */
